...
import { CHINAMONEY_JSON_ENDPOINT, ChinaMoneyJsonSource, syncChinaMoney } from "../src/modules/fx/index.js";
...
const pool = createPool(config.databaseUrl, "cli");
const requested = range(config.chinaMoneyHistoryStart ?? "2006-01-04");
log("info", "fx_sync_started", { kind: "MANUAL_RETRY", source: "ChinaMoney", ...requested });
try {
  const runId = await syncChinaMoney(pool, new ChinaMoneyJsonSource(CHINAMONEY_JSON_ENDPOINT), "MANUAL_RETRY", requested);
