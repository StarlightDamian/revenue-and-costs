...
import { applyTheme, getPendingThemePreference, normalizeTheme } from "./theme";
...
export function loadSession(force = false): Promise<void> {
  ...
  inFlight = (async () => {
    ...
    try {
      ...
      applyTheme(getPendingThemePreference() ?? normalizeTheme(session.me.theme));
    } catch (error) {
      session.me = null;
...
export function acceptSession(me: Me): void {
  ...
  session.status = "authenticated";
  session.error = "";
  applyTheme(getPendingThemePreference() ?? normalizeTheme(me.theme));
}
