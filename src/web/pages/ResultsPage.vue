...
<template>
  <section :class="props.embedded ? 'review-result-panel' : 'workflow-stage-page'" data-density="7">
    ...
    <AsyncState :status="status" :error="error" empty-title="暂无可查看结果" empty-message="所有者需要先完成资料准备和核算；客户只能看到已发布快照。" @retry="reload">
      ...
      <section class="version-strip" aria-label="结果固定版本"><div><span>数据</span><b>{{ report?.dataVersion }}</b></div><div><span>映射</span><b>{{ report?.mappingVersion }}</b></div><div><span>日期口径</span><b>{{ report?.timezoneVersion }}</b></div><div><span>策略</span><b>{{ report?.policyVersion }}</b></div><div><span>公式</span><b>{{ report?.formulaVersion }}</b></div><div><span>汇率</span><b>{{ report?.fxVersion }}</b></div><div><span>计算时间</span><b>{{ report?.calculatedAt }}</b></div></section>
    </AsyncState>
