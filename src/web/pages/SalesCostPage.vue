...
async function openCreateDialog() {
  ...
  catch (caught) { formError.value = userFacingError(caught, "暂时无法读取店铺价格，请检查网络后重试"); }
}
...
async function createShop() {
  ...
  if (!name.value.trim()) { formError.value = "请输入店铺名称"; return; }
  if (!creationOffer.value) { formError.value = "尚未读取当前店铺价格"; return; }
  saving.value = true;
  try {
    ...
    name.value = "";
    await reload();
  } catch (caught) { formError.value = userFacingError(caught, "暂时无法创建店铺，请检查网络后重试"); }
...
async function rename(shop: Shop) {
  ...
  catch (caught) { manageError.value = userFacingError(caught, "暂时无法修改店铺名称，请检查网络后重试"); }
...
async function changeLifecycle(shop: Shop, action: "TRASH" | "RESTORE") {
  manageError.value = "";
  if (!manageReason.value.trim()) { manageError.value = "删除或恢复店铺必须填写原因"; return; }
  managing.value = true;
  try {
  ...
  } catch (caught) { manageError.value = userFacingError(caught, "暂时无法修改店铺状态，请检查网络后重试"); }
  finally { managing.value = false; }
}
...
<template>
  <section>
    <PageHeader title="销售成本" description="按店铺查看资料是否齐全、计算结果和已发布的正式报告。" />

    <AsyncState :status="status" :error="error" empty-title="还没有可访问的店铺" empty-message="请先创建或加入企业；店铺客户授权会额外显示在这里。" @retry="reload">
      <template #empty-action><div class="empty-actions"><button v-if="canCreate" class="primary-button compact" type="button" @click="openCreateDialog">创建店铺</button><button class="tutorial-trigger compact" type="button" @click="openTutorial">获取资料教程</button></div></template>

      <div v-if="owned.length" class="surface-section shop-index-panel">
        <div class="shop-index-heading">
          <div class="section-heading"><h2>我的店铺</h2><p>选择店铺后可批量移入 30 天回收站；打开店铺后，可在资料准备、计算复核和报告交付三个页面完成工作。</p></div>
          <div class="shop-index-actions"><button v-if="canCreate" class="primary-button compact" type="button" @click="openCreateDialog">创建店铺</button><button class="tutorial-trigger compact" type="button" @click="openTutorial">获取资料教程</button></div>
          <div class="shop-filter" role="group" aria-label="店铺状态筛选"><button type="button" :class="{ 'is-active': shopFilter === 'ACTIVE' }" @click="shopFilter = 'ACTIVE'; selectedShopIds = new Set()">使用中</button><button type="button" :class="{ 'is-active': shopFilter === 'TRASHED' }" @click="shopFilter = 'TRASHED'; selectedShopIds = new Set()">回收站</button></div>
        </div>
        ...
        <div class="shop-selection-bar" :class="{ 'has-selection': selectedCount > 0 }">
          ...
          <span>已选择 {{ selectedCount }} 个店铺</span>
        ...
        <div class="shop-list">
          <article v-for="shop in visibleOwned" :key="shop.id" class="shop-row shop-row-redesigned">
            <label class="shop-check"><input type="checkbox" :checked="selectedShopIds.has(shop.id)" :aria-label="`选择店铺 ${shop.name}`" @change="toggleShop(shop.id)" /></label>
          ...
          <div v-if="visibleOwned.length === 0" class="workflow-empty-stage"><strong>{{ shopFilter === "TRASHED" ? "回收站为空" : "没有使用中的店铺" }}</strong><p>{{ shopFilter === "TRASHED" ? "删除后的店铺会在这里保留30天。" : "可以创建新店铺开始数据流程。" }}</p></div>
        </div>
      </div>
    ...
    <button v-if="managingShop" class="drawer-backdrop" type="button" aria-label="关闭店铺设置" @click="closeManagement"></button>
    <aside v-if="managingShop" class="shop-settings-drawer" aria-label="店铺设置">
      <header><div><span>店铺设置</span><h2>{{ managingShop.name }}</h2></div><button type="button" aria-label="关闭" @click="closeManagement">关闭</button></header>
      <div class="drawer-content">
        <div v-if="managingShop.status === 'TRASHED'" class="drawer-section"><p>该店铺位于30天回收站，恢复后保留原期限、数据版本和改名状态。</p><label class="form-field"><span>恢复原因</span><input v-model.trim="manageReason" /></label><button class="primary-button" type="button" :disabled="managing" @click="changeLifecycle(managingShop, 'RESTORE')">从回收站恢复</button></div>
        <template v-else>
          <section class="drawer-section"><h3>基本信息</h3><label class="form-field"><span>店铺名称</span><input v-model.trim="manageName" :disabled="!managingShop.renameAvailable" /></label><button class="secondary-button" type="button" :disabled="managing || !managingShop.renameAvailable" @click="rename(managingShop)">使用唯一一次改名</button><label class="form-field"><span>续期至（关闭日）</span><input v-model="renewCloseDate" type="date" /></label><button class="secondary-button" type="button" :disabled="managing" @click="renew(managingShop)">续期</button></section>
          ...
          <section class="drawer-section danger-zone"><h3>移入回收站</h3><p>店铺在30天内可以恢复，不退款，也不会物理删除财务事实。</p><label class="form-field"><span>删除原因</span><input v-model.trim="manageReason" /></label><button class="secondary-button" type="button" :disabled="managing" @click="changeLifecycle(managingShop, 'TRASH')">删除店铺</button></section>
        </template>
    ...
    <div v-if="showCreate" class="modal-layer" role="presentation" @keydown.esc="closeCreateDialog">
      <button class="modal-backdrop" type="button" aria-label="关闭创建店铺" @click="closeCreateDialog"></button>
      <form class="confirm-dialog create-shop-dialog" role="dialog" aria-modal="true" aria-labelledby="create-shop-title" @submit.prevent="createShop">
        <span>亚马逊销售成本</span>
        <h2 id="create-shop-title">创建店铺</h2>
        <p>{{ isAdmin ? "管理员创建店铺不扣企业钱包；系统仍会记录当时的原价和操作人，方便日后核对。" : "填写店铺信息后从当前企业钱包扣费。取消不会产生任何费用。" }}</p>
        <div class="form-grid three">
          <label class="form-field"><span>店铺名称</span><input v-model.trim="name" maxlength="120" autocomplete="off" autofocus /></label>
