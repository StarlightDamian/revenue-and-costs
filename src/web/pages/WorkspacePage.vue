...
<template>
  <section>
    ...
    <section class="workspace-overview-grid"><article class="surface-section"><span>当前企业</span><strong>{{ currentEnterprise?.name || "尚未创建" }}</strong><p>{{ enterpriseState.items.length }} 个可访问企业</p></article><article class="surface-section"><span>店铺</span><strong>{{ shops.length }}</strong><p>{{ shops.filter((shop) => shop.accountingStatus === "SUBMITTED").length }} 个已提交</p></article><article class="surface-section"><span>下一步</span><strong>{{ hasCompany ? "继续做账" : hasEnterprise ? "创建店铺" : "创建企业" }}</strong><RouterLink class="primary-button compact" :to="hasEnterprise ? '/sales-cost' : '/organization/enterprise'">前往处理</RouterLink></article></section>
  </section>
</template>
