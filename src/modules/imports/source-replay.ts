...
const MAX_REPLAY_FILES = 20_000;
const MAX_REPLAY_BYTES = 2n * 1024n * 1024n * 1024n;
const REPLAY_KEY_PREFIX = "admin-source-replay:";

interface CurrentSourceBindingRow {
...
function currentClosureHash(rows: readonly CurrentSourceBindingRow[]): string {
...
function validateAndDeduplicateSources(
  ...
  if (sources.length > MAX_REPLAY_FILES) throw new Error("SOURCE_REPLAY_FILE_LIMIT_EXCEEDED");
  ...
  if (totalBytes > MAX_REPLAY_BYTES) throw new Error("SOURCE_REPLAY_BYTE_LIMIT_EXCEEDED");
  return sources;
}
...
async function assertOperatorAndShop(
  ...
  const actor = await client.query<{ status: string; role: string }>(
    `SELECT account.status,role.role
       FROM account JOIN account_role role ON role.account_id=account.id
      WHERE account.id=$1`,
    [input.actorAccountId],
  );
  if (actor.rows[0]?.status !== "ACTIVE" || actor.rows[0]?.role !== "ADMIN") {
    throw new Error("SOURCE_REPLAY_ADMIN_REQUIRED");
  }
  const shop = await client.query<{ status: string }>("SELECT status FROM shop WHERE id=$1 FOR UPDATE", [input.shopId]);
  if (shop.rows[0]?.status !== "ACTIVE") throw new Error("SOURCE_REPLAY_SHOP_NOT_ACTIVE");
}
...
export async function replayCurrentShopSources(
  ...
  return withTransaction(pool, async (client) => {
    ...
    const sourceClosureHash = currentClosureHash(rows);
