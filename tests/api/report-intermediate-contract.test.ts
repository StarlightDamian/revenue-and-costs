...
describe("intermediate report HTTP contract", () => {
  ...
  it("streams a Chinese-header XLSX with numeric rates and formula-injection protection", async () => {
    const authorize = vi.fn(async () => undefined);
    ...
    const app = Fastify();
    await app.register(reportRoutes, {
      services: { getIntermediate, getIntermediateExportContext: async () => ({ enterpriseName: "测试企业", calculationRunId: "run-fixed", frozenRates }) } as never,
      authenticate: async () => actor,
      authorize,
      auditAdminAccess: async () => undefined,
    });
    ...
    expect(response.headers["content-disposition"]).toContain(encodeURIComponent("交易报告-测试企业.xlsx"));
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(response.rawPayload as never);
    const sheet = workbook.worksheets[0]!;
    expect(sheet.name).toBe("交易报告-测试企业");
    ...
    expect(getIntermediate).toHaveBeenNthCalledWith(1, shopId, "TRANSACTION", 200, undefined, {}, "run-fixed", frozenRates);
    expect(getIntermediate).toHaveBeenNthCalledWith(2, shopId, "TRANSACTION", 200, "1", {}, "run-fixed", frozenRates);
    await app.close();
  });
});
