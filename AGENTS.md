...
- 2026-08-13 当前受控试运行版本为 `/opt/revenue-costs/releases/20260813-025134/app`，制品 SHA-256 为 `388d22411b448c645d3693f47752cc4c857ca3d39e7521d661ddcb5102da96ca`，`/opt/revenue-costs/current` 原子指向该目录；Linux 离线依赖安装、API/Worker 启动、52 个迁移的文件名与校验和、数据库备份、Nginx 配置及回环/公开健康检查均已通过。本版本在保留上一版业务、权限和资源边界的基础上，修复深色原生下拉文字不可见，重构账号设置的信息层级与改名交互，移除设置页钱包卡片，并将导入、计算、结果、导出、钱包及管理页的用户提示改为可执行的白话说明；同时补齐上传完成失败后的恢复、结果完整性披露、内部枚举与错误隐藏以及迁移/应用数据库身份隔离门禁。生产官方汇率仍为 78,569 条、25 个币种、5,007 个开市日，覆盖 2006-01-04 至 2026-08-10，并保留 2 条当前人工覆盖。生产继续显式启用 `TEMPORARY_PUBLIC_REGISTRATION=true` 和 `PAYMENT_PROVIDER=temporary-manual`；真实注册账号只能获得 `ACCOUNTANT`，管理员授权继续使用应用现有逻辑。
...
- 当前版本切换前的数据库恢复点为 root-only `/var/backups/revenue-and-costs/pre-release-20260813-025134.dump`；上一可运行版本及其恢复点继续保留，失败发布产生的目录和重复备份已在确认非当前、非上一回滚版本后定向清理。`DATABASE_CAPACITY_PATH` 当前为 Worker 可访问且与 `pg_wal` 同卷的 `/var/lib/pgsql/17/data`，本次清理并完成新版本、备份及临时分片清理后验证可用空间为 11,982,401,536 字节。服务器无需云控制台即可通过已固定指纹的 SSH 直接管理；UDP 53/123 仍无响应、chrony 未同步，且 PostgreSQL 仍为 `archive_mode=off`、没有异机副本/WAL-PITR；用户已接受试运行期间暂不异地备份，所以 readiness 必须继续返回 `degraded`，不得宣称正式生产就绪。
