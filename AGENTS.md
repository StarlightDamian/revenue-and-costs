...
- 完整性键为“公司 + 规范化站点 + 报表字面月份 + 数据版本”，不以文件数、行数、订单数或 SKU 数是否相等判断。报表字面日期直接决定财务日期和月份，不转换到中国或站点时区；来源时区只用于生成可审计的绝对时间。任一已识别站点月份只要存在可解析配送货件，即使没有交易报告也按配送八字段独立计算；缺失交易侧不伪造退款、费用或零金额事实。没有配送货件时仅纯 FMB 交易切片可独立计算，其余缺失侧继续 `HARD_INCOMPLETE`，只能被确认排除且永不进入正式汇总。`SOFT_RECONCILIATION_WARNING` 仅在两类来源均存在时产生；当前标准导入以 `IMPORT_AUTO_WARNING_INCLUSION` 留下审计并带警告纳入，页面和导出持续披露。未知站点按大站点处理。
...
- 当前标准销售成本格式为 `revenue-and-costs-export-v10`，只含默认隐藏的 `口径说明`、`月度明细账单`、`季度明细账单`、`年度明细账单`、`成本核算表-人民币` 五个 Sheet；四个可见 Sheet 的常规有效行高为 30 磅，面向用户的主体表头统一为“店铺”，口径说明冻结本次可选核算月份，必要的多行说明行可以显式增高；完整性、费用与导入审计保留在数据库和页面，逐金额汇率使用保留在数据库审计链，生成前由服务端失败闭合校验。
...
- GitHub `main` 是可维护源码的唯一事实来源；服务器只保存从已合并 `main` 的干净提交构建出的不可变制品，不是 TypeScript/Vue 源码来源。运行事实必须现场核对 `/opt/revenue-costs/current`、版本目录中的 `.release-receipt.json`、数据库迁移清单和健康端点，不在本文件硬编码易过期的版本号、归档哈希、备份名、数据行数或剩余空间。
- 所有功能、修复和重构从已快进到 GitHub `main` 的干净工作树创建 `develop/<module>-<purpose>` 分支，经定向测试、`pnpm verify:release`、GitHub PR 与必需检查后合并；禁止直接在 `main` 开发、强推或从未提交工作树发布。发布脚本必须失败闭合校验本地分支为 `main`、工作树干净且 HEAD 等于 GitHub `origin/main`，并把 Git SHA 与制品哈希写入本地和远端发布回执。
...
- 每次切换前创建 root-only `/var/backups/revenue-and-costs/pre-release-<release-id>.dump`；成功后只保留当前版本、上一可运行版本及各自恢复点，删除前必须通过 realpath 明确排除 current 与 rollback 目标。`DATABASE_CAPACITY_PATH` 必须是 Worker 可访问且与 `pg_wal` 同卷的 `/var/lib/pgsql/17/data`。服务器仍无异机副本/WAL-PITR且时钟同步条件未闭合，所以 readiness 必须继续返回 `degraded`，不得宣称正式生产就绪。
...
- 首次生产初始化只允许使用全新空数据库：执行当前前向迁移和内置映射初始化后，只创建用户明确确认的一个管理员账号；后续发布只对现有生产库执行已校验的前向迁移。任何阶段都禁止复制本地、测试或 Googcci 的账号、企业、公司、钱包、订单、导入、文件、任务和审计数据；首次启用前必须用数据库查询证明除该管理员及必要系统配置外业务表为空。
...
- 每次发布按“本地 `pnpm verify:release` 与适用的 `pnpm test:recovery` → 干净 `main`/远端 HEAD 一致性 → 本地离线制品与发布回执 → 服务器新版本目录 → 数据库备份与前向迁移 → 原子切换 systemd/Nginx → `/health/live`、粗粒度 `/health/ready`、关键登录、Worker 心跳和稳定窗口验收”推进。失败时先原子回到上一代码版本；数据库只用前向修复或已演练恢复，不回写迁移历史。生产数据、对象和秘密不得反向同步到本地测试环境。
