...
- 当前标准销售成本格式为 `revenue-and-costs-export-v9`，只含默认隐藏的 `口径说明`、`月度明细账单`、`季度明细账单`、`年度明细账单`、`成本核算表-人民币` 五个 Sheet；四个可见 Sheet 的常规有效行高为 30 磅，必要的多行说明行可以显式增高；完整性、费用与导入审计保留在数据库和页面，逐金额汇率使用保留在数据库审计链，生成前由服务端失败闭合校验。
